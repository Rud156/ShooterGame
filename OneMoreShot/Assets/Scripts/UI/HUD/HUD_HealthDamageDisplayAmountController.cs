...
namespace UI.HUD
{
    ...
    public class HUD_HealthDamageDisplayAmountController : MonoBehaviour
    {
        ...
        private void Update()
        {
            for (var i = _widgetData.Count - 1; i >= 0; i--)
            {
                ...
                var widgetExists = HUD_HealthDamageDisplayAmount.Instance.UpdateWidgetPosition(_widgetData[i].WidgetId, mappedPoint);
                if (!widgetExists)
                {
        ...
        private void HandleDamageTaken(int damageTaken, int startingHealth, int finalHealth)
        {
            ...
            var widgetId = HUD_HealthDamageDisplayAmount.Instance.AddDamageWidget(mappedPoint, damageTaken);
        ...
        private void HandleHealed(int healAmount, int startingHealth, int finalHealth)
        {
            ...
            var widgetId = HUD_HealthDamageDisplayAmount.Instance.AddHealWidget(mappedPoint, healAmount);
            _widgetData.Add(new DisplayWidgetData
            {
                WidgetId = widgetId,
                WidgetWorldPosition = widgetWorldPosition
